...
from graphite.solvers.hybrid_genetic_solver import HybridGeneticSolver
...
def main():
    ...
    test_solvers = [HybridGeneticSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "hybrid_genetic_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "hybrid_genetic_scores.csv"))

    algo_scores = scores_dict['HybridGeneticSolver']
    ...
    print(f"\nHybridGeneticSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ HybridGeneticSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ HybridGeneticSolver is not better than NearestNeighbourSolver")
