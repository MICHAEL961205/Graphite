...
from graphite.solvers.nearest_insertion_two_opt_solver import NearestInsertionTwoOptSolver
...
def main():
    ...
    test_solvers = [NearestInsertionTwoOptSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "nearest_insertion_two_opt_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "nearest_insertion_two_opt_scores.csv"))

    algo_scores = scores_dict['NearestInsertionTwoOptSolver']
    ...
    print(f"\nNearestInsertionTwoOptSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ NearestInsertionTwoOptSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ NearestInsertionTwoOptSolver is not better than NearestNeighbourSolver")
