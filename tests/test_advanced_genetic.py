...
from graphite.solvers.advanced_genetic_solver import AdvancedGeneticSolver
...
N_PROBLEMS = 6  # Smaller for complex algorithm
...
def main():
    ...
    test_solvers = [AdvancedGeneticSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "advanced_genetic_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "advanced_genetic_scores.csv"))

    algo_scores = scores_dict['AdvancedGeneticSolver']
    ...
    print(f"\nAdvancedGeneticSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ AdvancedGeneticSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ AdvancedGeneticSolver is not better than NearestNeighbourSolver")
