...
from graphite.solvers.dfj_solver import DFJSolver
...
def main():
    ...
    test_solvers = [DFJSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "dfj_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "dfj_scores.csv"))

    algo_scores = scores_dict['DFJSolver']
    ...
    print(f"\nDFJSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ DFJSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ DFJSolver is not better than NearestNeighbourSolver")
