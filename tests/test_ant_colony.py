...
from graphite.solvers.ant_colony_solver import AntColonySolver
...
def main():
    ...
    test_solvers = [AntColonySolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "ant_colony_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "ant_colony_scores.csv"))

    algo_scores = scores_dict['AntColonySolver']
    ...
    print(f"\nAntColonySolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ AntColonySolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ AntColonySolver is not better than NearestNeighbourSolver")
