...
from graphite.solvers.greedy_cycle_solver import GreedyCycleSolver
...
def main():
    ...
    test_solvers = [GreedyCycleSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "greedy_cycle_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "greedy_cycle_scores.csv"))

    algo_scores = scores_dict['GreedyCycleSolver']
    ...
    print(f"\nGreedyCycleSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ GreedyCycleSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ GreedyCycleSolver is not better than NearestNeighbourSolver")
