...
from graphite.solvers.multi_objective_genetic_solver import MultiObjectiveGeneticSolver
...
def main():
    ...
    test_solvers = [MultiObjectiveGeneticSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "multi_objective_genetic_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "multi_objective_genetic_scores.csv"))

    algo_scores = scores_dict['MultiObjectiveGeneticSolver']
    ...
    print(f"\nMultiObjectiveGeneticSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ MultiObjectiveGeneticSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ MultiObjectiveGeneticSolver is not better than NearestNeighbourSolver")
