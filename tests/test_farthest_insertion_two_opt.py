...
from graphite.solvers.farthest_insertion_two_opt_solver import FarthestInsertionTwoOptSolver
...
def main():
    ...
    test_solvers = [FarthestInsertionTwoOptSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "farthest_insertion_two_opt_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "farthest_insertion_two_opt_scores.csv"))

    algo_scores = scores_dict['FarthestInsertionTwoOptSolver']
    ...
    print(f"\nFarthestInsertionTwoOptSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ FarthestInsertionTwoOptSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ FarthestInsertionTwoOptSolver is not better than NearestNeighbourSolver")
