...
from graphite.solvers.variable_neighborhood_solver import VariableNeighborhoodSolver
...
def main():
    ...
    test_solvers = [VariableNeighborhoodSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "variable_neighborhood_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "variable_neighborhood_scores.csv"))

    algo_scores = scores_dict['VariableNeighborhoodSolver']
    ...
    print(f"\nVariableNeighborhoodSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ VariableNeighborhoodSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ VariableNeighborhoodSolver is not better than NearestNeighbourSolver")
