...
from graphite.solvers.clarke_wright_solver import ClarkeWrightSolver
...
def main():
    ...
    test_solvers = [ClarkeWrightSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "clarke_wright_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "clarke_wright_scores.csv"))

    algo_scores = scores_dict['ClarkeWrightSolver']
    ...
    print(f"\nClarkeWrightSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ ClarkeWrightSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ ClarkeWrightSolver is not better than NearestNeighbourSolver")
