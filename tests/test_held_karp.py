...
from graphite.solvers.held_karp_solver import HeldKarpSolver
...
def main():
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "held_karp_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "held_karp_scores.csv"))

    algo_scores = scores_dict['HeldKarpSolver']
    ...
    print(f"\nHeldKarpSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ HeldKarpSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ HeldKarpSolver is not better than NearestNeighbourSolver")
