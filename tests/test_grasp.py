...
from graphite.solvers.grasp_solver import GRASPSolver
...
def main():
    ...
    test_solvers = [GRASPSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "grasp_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "grasp_scores.csv"))

    algo_scores = scores_dict['GRASPSolver']
    ...
    print(f"\nGRASPSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ GRASPSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ GRASPSolver is not better than NearestNeighbourSolver")
