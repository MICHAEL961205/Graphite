...
from graphite.solvers.memetic_solver import MemeticSolver
...
def main():
    ...
    test_solvers = [MemeticSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "memetic_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "memetic_scores.csv"))

    algo_scores = scores_dict['MemeticSolver']
    ...
    print(f"\nMemeticSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ MemeticSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ MemeticSolver is not better than NearestNeighbourSolver")
