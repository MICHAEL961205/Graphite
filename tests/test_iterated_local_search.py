...
from graphite.solvers.iterated_local_search_solver import IteratedLocalSearchSolver
...
def main():
    ...
    test_solvers = [IteratedLocalSearchSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "iterated_local_search_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "iterated_local_search_scores.csv"))

    algo_scores = scores_dict['IteratedLocalSearchSolver']
    ...
    print(f"\nIteratedLocalSearchSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ IteratedLocalSearchSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ IteratedLocalSearchSolver is not better than NearestNeighbourSolver")
