...
from graphite.solvers.ai_tsp_solver import AttentionModelSolver
...
def main():
    ...
    test_solvers = [AttentionModelSolver(), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "ai_tsp_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "ai_tsp_scores.csv"))

    algo_scores = scores_dict['AttentionModelSolver']
    ...
    print(f"\nAttentionModelSolver improvements: {improvements}/{comparable}")
    avg_algo = np.mean([s for s in algo_scores if s != float('inf')]) if comparable else float('inf')
    avg_nn = np.mean([s for s in nn_scores if s != float('inf')]) if comparable else float('inf')
    print(f"Average algo score: {avg_algo:.2f}")
    print(f"Average NN score: {avg_nn:.2f}")
    if improvements > 0:
        print(f"✅ AttentionModelSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ AttentionModelSolver is not better than NearestNeighbourSolver")
