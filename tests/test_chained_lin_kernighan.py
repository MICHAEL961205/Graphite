...
from graphite.solvers.chained_lin_kernighan_solver import ChainedLinKernighanSolver
...
def main():
    ...
    test_solvers = [ChainedLinKernighanSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "chained_lin_kernighan_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "chained_lin_kernighan_scores.csv"))

    algo_scores = scores_dict['ChainedLinKernighanSolver']
    ...
    print(f"\nChainedLinKernighanSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ ChainedLinKernighanSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ ChainedLinKernighanSolver is not better than NearestNeighbourSolver")
