...
from graphite.solvers.mst_doubling_solver import MSTDoublingSolver
...
def main():
    ...
    test_solvers = [MSTDoublingSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "mst_doubling_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "mst_doubling_scores.csv"))

    algo_scores = scores_dict['MSTDoublingSolver']
    ...
    print(f"\nMSTDoublingSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ MSTDoublingSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ MSTDoublingSolver is not better than NearestNeighbourSolver")
