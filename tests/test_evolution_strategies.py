...
from graphite.solvers.evolution_strategies_solver import EvolutionStrategiesSolver
...
def main():
    ...
    test_solvers = [EvolutionStrategiesSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "evolution_strategies_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "evolution_strategies_scores.csv"))

    algo_scores = scores_dict['EvolutionStrategiesSolver']
    ...
    print(f"\nEvolutionStrategiesSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ EvolutionStrategiesSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ EvolutionStrategiesSolver is not better than NearestNeighbourSolver")
