...
def main():
    ...
    test_solvers = [LKHSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "lkh_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "lkh_scores.csv"))

    algo_scores = scores_dict['LKHSolver']
    ...
    print(f"\nLKHSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ LKHSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ LKHSolver is not better than NearestNeighbourSolver")
