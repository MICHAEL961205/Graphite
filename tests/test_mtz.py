...
from graphite.solvers.mtz_solver import MTZSolver
...
def main():
    ...
    test_solvers = [MTZSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "mtz_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "mtz_scores.csv"))

    algo_scores = scores_dict['MTZSolver']
    ...
    print(f"\nMTZSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ MTZSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ MTZSolver is not better than NearestNeighbourSolver")
