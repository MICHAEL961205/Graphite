...
N_PROBLEMS = 8  # Smaller for complex algorithm
...
def main():
    ...
    test_solvers = [LinKernighanSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "lin_kernighan_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "lin_kernighan_scores.csv"))

    algo_scores = scores_dict['LinKernighanSolver']
    ...
    print(f"\nLinKernighanSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ LinKernighanSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ LinKernighanSolver is not better than NearestNeighbourSolver")
