...
from graphite.solvers.advanced_simulated_annealing_solver import AdvancedSimulatedAnnealingSolver
...
def main():
    ...
    test_solvers = [AdvancedSimulatedAnnealingSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "advanced_simulated_annealing_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "advanced_simulated_annealing_scores.csv"))

    algo_scores = scores_dict['AdvancedSimulatedAnnealingSolver']
    ...
    print(f"\nAdvancedSimulatedAnnealingSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ AdvancedSimulatedAnnealingSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ AdvancedSimulatedAnnealingSolver is not better than NearestNeighbourSolver")
