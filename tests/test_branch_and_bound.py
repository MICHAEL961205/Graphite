...
from graphite.solvers.branch_and_bound_solver import BranchAndBoundSolver
...
def main():
    ...
    test_solvers = [BranchAndBoundSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "branch_and_bound_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "branch_and_bound_scores.csv"))

    algo_scores = scores_dict['BranchAndBoundSolver']
    ...
    print(f"\nBranchAndBoundSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ BranchAndBoundSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ BranchAndBoundSolver is not better than NearestNeighbourSolver")
