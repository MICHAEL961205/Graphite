...
from graphite.solvers.concorde_hybrid_solver import ConcordeHybridSolver
...
def main():
    ...
    test_solvers = [ConcordeHybridSolver(time_limit=100), NearestNeighbourSolver()]
    ...
    run_times_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "concorde_hybrid_run_times.csv"))
    scores_df.to_csv(os.path.join(ROOT_DIR, SAVE_DIR, "concorde_hybrid_scores.csv"))

    algo_scores = scores_dict['ConcordeHybridSolver']
    ...
    print(f"\nConcordeHybridSolver improvements: {improvements}/{comparable}")
    ...
    if improvements > 0:
        print(f"✅ ConcordeHybridSolver is better than NearestNeighbourSolver!")
    else:
        print(f"❌ ConcordeHybridSolver is not better than NearestNeighbourSolver")
